...
from vq_vae_small import AutoEncoder, to_scaler
...
def generate_episodic_results(base_path,data_loader,nr_logistic_mix,do_use_cuda):
    ...
    for batch_idx, (data, img_names) in enumerate(data_loader):
        ...
        # should remove
        # only need name of first image
        basename =  os.path.split(img_names[0])[1].replace('.png', '_vqvae.npz').replace('_frame00000', '')
        iname = os.path.join(base_path, basename)
...
if __name__ == '__main__':
    ...
    if args.model_loadpath is not None:
        ...
        if os.path.exists(args.model_loadpath):
            ...
            vqvae.load_state_dict(model_dict['state_dict'])
